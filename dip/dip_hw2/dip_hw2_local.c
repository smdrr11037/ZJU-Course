...
int main()
{
    ...
    for(int m=0;m<row;m++){//m表示窗口在第几行
        for(int n=0;n<column;n++){//n表示窗口在第几列
            for(int i=1;i<max-min;i++){
                int fore_num=0;//计数前景像素数
                double fore_Y=0;//统计前景灰度总和
                int back_num=0;//计数后景像素数
                double back_Y=0;//统计后景灰度总和
                for(int k=0;k<bmih.biHeight/row;k++){//像素在小窗口的第k行
                    for(int j=0;j<bmih.biWidth/column;j++){//像素在小窗口的第j列
                        //Y值<threshold(min+i),前景像素数++，反之后景++
                        //计算当前像素值的下标
                        int subscript=m*bmih.biWidth*bmih.biHeight/row+bmih.biWidth/column*n+j+k*bmih.biWidth;
            ...
            //大津算法计算方差
                double temp=0;
                temp=fore_num*back_num*1.0/pixel_num*column*row/pixel_num*column*row*
